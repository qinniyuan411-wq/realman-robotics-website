(function () {
  ...
  // 自建后端识别（阿里云北京机器：直连 IP 测试期 / 上线后正式域名）。
  // 命中此分支时表单走同源 /api/submit-contact，由 Nginx 反代到 Node Express。
  var isSelfHosted = (
  ...
  var ENDPOINT = isSelfHosted
    ? { type: 'self', url: '/api/submit-contact', lang: 'cn' }
    : isLocal
  ...
  // Cloudflare Turnstile site key（公开值，可前端可见；secret key 仅在后端 env vars）
  // 对应 hostnames：qinnitest.you / localhost / 127.0.0.1 / *.realman-robotics.com
  var TURNSTILE_SITE_KEY = '0x4AAAAAADA6F3o-puTE8mWb';
  // 腾讯云验证码 CaptchaAppId（公开值，可前端可见；AppSecretKey 仅在后端）
  // 申请地址：https://console.cloud.tencent.com/captcha
  // 占位 __NEED_TENCENT_APPID__ 表示尚未配置；为占位时不注入腾讯验证码组件。
  var TENCENT_CAPTCHA_APP_ID = '__NEED_TENCENT_APPID__';

  // 验证码方案探测：
  //   1) URL 强制：?captcha=tencent | turnstile | none （仅测试期使用）
  //   2) HOST 自动：
  //      .cn 域名 / 测试 IP 的 HTTPS  → 腾讯验证码（国内稳定）
  //      .com 域名 / qinnitest.you    → Cloudflare Turnstile（海外稳定）
  //      测试 IP 的 HTTP              → 跳过（HTTP 环境两种都会失败）
  //      其他                         → 跳过
  function detectCaptchaProvider () {
    ...
    } catch (_) { /* URLSearchParams may be missing on very old browsers */ }
  ...
  // 按 CAPTCHA_PROVIDER 注入对应组件。
  // - turnstile：内嵌渲染，提交时同步读取 token
  // - tencent  ：仅加载 SDK 脚本，提交时弹窗触发，无需在按钮旁占位
  // - none     ：不注入任何组件
  ...
  // 统一的验证码 token 获取（Promise）。在表单 submit 时调用。
  function getCaptchaToken (provider) {
    return new Promise(function (resolve, reject) {
      if (provider === 'turnstile') {
        ...
        if (!ts) return reject(new Error('请先完成人机验证。'));
        return resolve({ 'cf-turnstile-response': ts });
      }
      if (provider === 'tencent') {
        if (!window.TencentCaptcha) return reject(new Error('验证组件加载中，请稍后重试。'));
        if (!TENCENT_CAPTCHA_APP_ID || TENCENT_CAPTCHA_APP_ID.indexOf('__') === 0) {
          return reject(new Error('验证码尚未配置（管理员请填写 TENCENT_CAPTCHA_APP_ID）。'));
        }
        try {
          var captcha = new window.TencentCaptcha(TENCENT_CAPTCHA_APP_ID, function (res) {
            ...
            if (res && res.ret === 0) {
              ...
              reject(new Error(res && res.ret === 2 ? '已取消验证。' : '验证未通过，请重试。'));
            }
          });
          captcha.show();
        } catch (err) {
          reject(new Error('验证组件初始化失败：' + (err && err.message || err)));
        }
        return;
      }
      // none
  ...
  form.addEventListener('submit', async function (e) {
    ...
    // 蜜罐字段：人眼不可见、键盘不可达；只有自动化脚本会盲填，填了即为机器人
    ...
    if (ENDPOINT.type === 'edge' || ENDPOINT.type === 'self') {
      ...
      // 按 provider 异步取 token（无 provider 时直接 resolve {}）
      var captchaProvider = CAPTCHA_PROVIDER;
      // 历史兼容：原 Supabase Edge 强制 Turnstile；走 'edge' 路径时不接受 'none'
      ...
      try {
        ...
        return fail((err && err.message) || '人机验证失败。');
      }
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        var hint = ({
          origin_not_allowed:        '当前域名未授权（联系管理员）',
          unsupported_media_type:    '请求格式错误',
          rate_limited:              '操作过于频繁，请 1 分钟后重试',
          invalid_json:              '请求格式错误',
          missing_captcha:           '请先完成人机验证',
          captcha_provider_unavailable: '验证码服务暂未配置',
          turnstile_failed:          '人机验证未通过，请重试',
          tencent_captcha_failed:    '验证码未通过，请重试',
          missing_required:          '请填写所有必填项',
          invalid_name:              '姓名格式不正确',
          invalid_email:             '邮箱格式不正确',
          company_too_long:          '公司名称过长',
          details_too_long:          '详细内容过长',
          forbidden_content:         '内容包含禁止字符',
          invalid_inquiry:           '请选择有效的咨询类型',
          invalid_region:            '请选择有效的地区',
          invalid_sub_region:        '请选择有效的省份/区域',
          invalid_overseas:          '请选择有效的海外区域',
          invalid_lang:              '语言标识错误',
          db_insert_failed:          '服务器写入失败，请稍后重试',
        })[serverMsg] || ('提交失败 (HTTP_' + res.status + (serverMsg ? ' / ' + serverMsg : '') + ')');
      ...
      // 腾讯验证码：每次 new TencentCaptcha 都是独立实例，无需 reset。
      setTimeout(function () {
        restoreBtn(btn, btnSnapshot);
        btn.disabled = false;
