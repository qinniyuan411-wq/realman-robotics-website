(function () {
  ...
  // Self-hosted backend (Aliyun Beijing single-node): direct IP for staging, official domain in prod.
  // When matched, form posts same-origin /api/submit-contact (Nginx → Node Express).
  var isSelfHosted = (
    location.hostname === '123.56.218.87' ||
    /(^|\.)realman-robotics\.com$/i.test(location.hostname) ||
    /(^|\.)realman-robotics\.cn$/i.test(location.hostname)
  );

  // Form submission endpoint: local → Supabase REST; self-hosted → /api/; otherwise → Supabase Edge.
  var ENDPOINT = isSelfHosted
    ? { type: 'self', url: '/api/submit-contact', lang: 'en' }
    : isLocal
  ...
  // Cloudflare Turnstile site key (public; secret key lives only in backend env)
  var TURNSTILE_SITE_KEY = '0x4AAAAAADA6F3o-puTE8mWb';
  // Tencent Cloud Captcha CaptchaAppId (public; AppSecretKey lives only in backend)
  // Register at https://console.cloud.tencent.com/captcha
  // While set to __NEED_TENCENT_APPID__ the widget is not injected.
  var TENCENT_CAPTCHA_APP_ID = '__NEED_TENCENT_APPID__';

  // Captcha provider detection:
  //   1) URL override: ?captcha=tencent | turnstile | none  (test only)
  //   2) Hostname auto:
  //      .cn or test IP HTTPS  → tencent (stable in mainland China)
  //      .com / qinnitest.you  → turnstile (stable globally)
  //      test IP HTTP          → none (both providers fail on HTTP)
  function detectCaptchaProvider () {
    try {
      var override = new URLSearchParams(location.search).get('captcha');
      if (override === 'tencent' || override === 'turnstile' || override === 'none') return override;
    } catch (_) {}
    var host = location.hostname;
    var isHttps = location.protocol === 'https:';
    if (host === '123.56.218.87') return 'none';
    if (/(^|\.)realman-robotics\.cn$/i.test(host)) return 'none';
    if (/(^|\.)realman-robotics\.com$/i.test(host)) return 'none';
    if (host === 'qinnitest.you') return 'turnstile';
    return 'none';
  }
  var CAPTCHA_PROVIDER = detectCaptchaProvider();
  ...
  // Inject the captcha widget for the active provider.
  if (CAPTCHA_PROVIDER === 'turnstile' && TURNSTILE_SITE_KEY && TURNSTILE_SITE_KEY.indexOf('__') !== 0) {
    var submitBtnT = form.querySelector('button[type="submit"]');
    if (submitBtnT && !document.getElementById('cf-turnstile-wrap')) {
      ...
      submitBtnT.parentNode.insertBefore(wrap, submitBtnT);
    }
  ...
  } else if (CAPTCHA_PROVIDER === 'tencent' && TENCENT_CAPTCHA_APP_ID && TENCENT_CAPTCHA_APP_ID.indexOf('__') !== 0) {
    if (!document.querySelector('script[data-tencent-captcha]')) {
      var ts = document.createElement('script');
      ts.src = 'https://turing.captcha.qcloud.com/TCaptcha.js';
      ts.async = true;
      ts.defer = true;
      ts.setAttribute('data-tencent-captcha', '1');
      document.head.appendChild(ts);
    }
  }

  // Unified async captcha-token getter; called inside form submit handler.
  function getCaptchaToken (provider) {
    return new Promise(function (resolve, reject) {
      if (provider === 'turnstile') {
        var node = form.querySelector('[name="cf-turnstile-response"]');
        var ts = (window.turnstile && node) ? node.value : '';
        if (!ts) return reject(new Error('Please complete the human verification.'));
        return resolve({ 'cf-turnstile-response': ts });
      }
      if (provider === 'tencent') {
        if (!window.TencentCaptcha) return reject(new Error('Verification widget loading, please retry.'));
        if (!TENCENT_CAPTCHA_APP_ID || TENCENT_CAPTCHA_APP_ID.indexOf('__') === 0) {
          return reject(new Error('Captcha not configured (admin must set TENCENT_CAPTCHA_APP_ID).'));
        }
        try {
          var captcha = new window.TencentCaptcha(TENCENT_CAPTCHA_APP_ID, function (res) {
            if (res && res.ret === 0) {
              resolve({ tencent_ticket: res.ticket, tencent_randstr: res.randstr });
            } else {
              reject(new Error(res && res.ret === 2 ? 'Verification cancelled.' : 'Verification failed, please retry.'));
            }
          });
          captcha.show();
        } catch (err) {
          reject(new Error('Captcha init failed: ' + (err && err.message || err)));
        }
        return;
      }
      resolve({});
  ...
  form.addEventListener('submit', async function (e) {
    ...
    // Honeypot field: invisible to humans, only filled by automated scripts.
    var honeypotEl = form.querySelector('input[name="company_url"]');
    ...
    var payload = {
      ...
      company_url: honeypotVal
    };

    if (ENDPOINT.type === 'edge' || ENDPOINT.type === 'self') {
      payload.lang = ENDPOINT.lang;
      var captchaProvider = CAPTCHA_PROVIDER;
      // Legacy Supabase Edge always required Turnstile; do not allow 'none' on that path.
      if (ENDPOINT.type === 'edge' && captchaProvider === 'none') captchaProvider = 'turnstile';
      try {
        var captchaFields = await getCaptchaToken(captchaProvider);
        Object.keys(captchaFields).forEach(function (k) { payload[k] = captchaFields[k]; });
      } catch (err) {
        return fail((err && err.message) || 'Captcha failed.');
      }
    }
    ...
    try {
      ...
      if (!res.ok) {
        var serverMsg = '';
        try { var j = await res.json(); serverMsg = j && j.error ? String(j.error) : ''; } catch (_) {}
        var hint = ({
          origin_not_allowed:        'This domain is not authorized (contact admin).',
          unsupported_media_type:    'Bad request format.',
          rate_limited:              'Too many submissions. Please wait 1 minute.',
          invalid_json:              'Bad request format.',
          missing_captcha:           'Please complete the human verification.',
          captcha_provider_unavailable: 'Captcha service not configured.',
          turnstile_failed:          'Verification failed, please retry.',
          tencent_captcha_failed:    'Verification failed, please retry.',
          missing_required:          'Please fill in all required fields.',
          invalid_name:              'Invalid name format.',
          invalid_email:             'Invalid email format.',
          company_too_long:          'Company name too long.',
          details_too_long:          'Details too long.',
          forbidden_content:         'Forbidden characters in content.',
          invalid_inquiry:           'Please select a valid inquiry type.',
          invalid_region:            'Please select a valid region.',
          invalid_lang:              'Language tag error.',
          db_insert_failed:          'Server write failed, please retry later.',
        })[serverMsg] || ('Submission failed (HTTP_' + res.status + (serverMsg ? ' / ' + serverMsg : '') + ')');
      ...
      fail((err && err.message) || 'Submission failed. Please try again later.');
    }
  });
})();
